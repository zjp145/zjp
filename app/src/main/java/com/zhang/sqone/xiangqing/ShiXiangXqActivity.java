...
//        hyapCx.setVisibility(View.VISIBLE);
    ...
    private void regRequest2() {
        final Meetingroom.ReqMeetingRoom index = Meetingroom.ReqMeetingRoom.newBuilder().setSid(User.sid).setAc("SXTZXQ").setId(xqid).build();
        new UniversalHttp() {
            @Override
...
//                        Log.i("zhang", "sd卡的路径：" + Environment.getExternalStorageDirectory().getPath()+"/"+filename);
                                ...
                                XiaZaiUtil xiaZaiUtil = new XiaZaiUtil(holder.<ImageButton>getView(R.id.xiazai_iamge_button), ShiXiangXqActivity.this, ceshi.getUrl(), holder.<MasterLayout>getView(R.id.xiazai_zx), holder.<TextView>getView(R.id.xiazai_sudu), ceshi.getName());
//                        Log.i("zhang", ceshi.getFt());
                        ...
                        Toast.makeText(ShiXiangXqActivity.this, index.getMsg(), Toast.LENGTH_SHORT).show();
                    }

                }
            }
        }.protocolBuffer(ShiXiangXqActivity.this, Globals.HYAP_URI, null);
    }
