...
//        listAdd();
    ...
    private void regRequest() {
        final Outbox.ReqOutBox index = Outbox.ReqOutBox.newBuilder().setSid(User.sid).setId(xqid).setRid(rid).setAc("SZLXQ").build();
        new UniversalHttp() {
            @Override
...
//                        Log.i("zhang", "sd卡的路径：" + Environment.getExternalStorageDirectory().getPath()+"/"+filename);
                                ...
                                XiaZaiUtil xiaZaiUtil = new XiaZaiUtil(holder.<ImageButton>getView(R.id.xiazai_iamge_button), ShouYouJianXQActivity.this, ceshi.getUrl(), holder.<MasterLayout>getView(R.id.xiazai_zx), holder.<TextView>getView(R.id.xiazai_sudu),ceshi.getName());
//                        Log.i("zhang", ceshi.getFt());
        ...
        }.protocolBuffer(ShouYouJianXQActivity.this, Globals.ZL_URI, null);
    ...
    public void onClick(View view) {
        switch (view.getId()){
            case R.id.shouyoujian_shenchu:
                final Outbox.ReqOutBox index = Outbox.ReqOutBox.newBuilder().setSid(User.sid).setRid(rid).setId(xqid).setAc("DLSZL").build();
                new UniversalHttp() {
                    @Override
                    public <T> void outPutInterface(OutputStream outputStream) throws IOException {
                        index.writeTo(outputStream);
