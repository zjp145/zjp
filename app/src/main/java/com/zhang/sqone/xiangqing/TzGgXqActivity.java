...
public class TzGgXqActivity extends BaseActivity {
     ...
     * 下载附件的集合
     */
            ListViewForScrollView xiangqingList;

    @Bind(R.id.xiangqing_con1)
    /**文件内容*/
            WebView xiangqingCon1;
    @Bind(R.id.tzggxq_title)
    TitleBarView tzggxqTitle;
    @Bind(R.id.tzgg_iamge)
    ImageView tzggIamge;
    @Bind(R.id.tzgg_title)
    TextView tzggTitle;
    @Bind(R.id.tzgg_time)
    TextView tzggTime;
    @Bind(R.id.tzgg_con)
     ...
     * 创建数据集合测试适配器
     */
    private List<Noticecontacts.ReqNoticeAndContacts.FileList> arrayList;
    /**
     * 数据适配器
     */
    private CommonAdapter<Noticecontacts.ReqNoticeAndContacts.FileList> ja;
    /**
     * 详情id
     */
    private String xqid;
    private Noticecontacts.ReqNoticeAndContacts.NoticeMap fileMap;

    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setTranslucentStatus();
        setContentView(R.layout.activity_tz_gg_xq);
        ButterKnife.bind(this);
        fileMap = (Noticecontacts.ReqNoticeAndContacts.NoticeMap) getIntent().getSerializableExtra(Globals.LIDS);
//        ImageLoader.getInstance().displayImage(fileMap.getPurl(), tzggIamge);
        tzggTitle.setText(fileMap.getT());
        tzggTime.setText(fileMap.getDt());
        xqid = fileMap.getId();
        Log.i("zhang", "xqid++"+xqid);
        Log.i("zhang", "读取状态"+fileMap.getState());
        if (fileMap.getState().equals("0")){
            tzggIamge.setImageResource(R.mipmap.tzgg_weidu);
        }else{
            tzggIamge.setImageResource(R.mipmap.tzgg_yidu);
        }
        //加载数据
        regRequest();
//        listAdd();
    ...
    private void regRequest() {
        final Noticecontacts.ReqNoticeAndContacts index = Noticecontacts.ReqNoticeAndContacts.newBuilder().setSid(User.sid).setId(xqid).setAc("TZXQ").build();
        new UniversalHttp() {
            @Override
...
//                xiangqingCon2.setText(index.getPa().getCon());
                        ...
                        //创建适配器对布局控件添加属性
                        ja = new CommonAdapter<Noticecontacts.ReqNoticeAndContacts.FileList>(TzGgXqActivity.this, arrayList, R.layout.xia_zai_item) {
                            @Override
                            public void convert(ViewHolder holder, Noticecontacts.ReqNoticeAndContacts.FileList ceshi) {
                                ...
                                if (AppUtil.fileIsExists(Environment.getExternalStorageDirectory() + "/zhtzwj/" + filename)) {
                                    ...
                                    imagebutton.setOnClickListener(new View.OnClickListener() {
                                        ...
                                        public void onClick(View v) {
                                            ...
                                            TzGgXqActivity.this.startActivity(intent);
                                        }
                                    });
...
//                        Log.i("zhang", "sd卡的路径：" + Environment.getExternalStorageDirectory().getPath()+"/"+filename);
                                ...
                                XiaZaiUtil xiaZaiUtil = new XiaZaiUtil(holder.<ImageButton>getView(R.id.xiazai_iamge_button), TzGgXqActivity.this, ceshi.getUrl(), holder.<MasterLayout>getView(R.id.xiazai_zx), holder.<TextView>getView(R.id.xiazai_sudu),ceshi.getName());
//                        Log.i("zhang", ceshi.getFt());
                        ...
                        Toast.makeText(TzGgXqActivity.this, index.getMsg(), Toast.LENGTH_SHORT).show();
                    }

                }
            }
        }.protocolBuffer(TzGgXqActivity.this, Globals.TZTX_URI, null);
